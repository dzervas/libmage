...
mod tests {
    ...
    #[test]
    fn new() {
        let file = File::create(if cfg!(target_os = "windows") { "%TEMP%\\mage-test" } else { "/tmp/mage-test" }).unwrap();
...
//        assert!(Connection::new(0x1FFFFFF, Box::new(file.try_clone().unwrap()), &mut rw, true, &[1; 32], &[2; 32]).is_err(), "ID is longer than 3 bytes");
    ...
    #[test]
    fn read_write() {
        let file = OpenOptions::new().read(true).write(true).create(true).open(if cfg!(target_os = "windows") { "%TEMP%\\mage-test" } else { "/tmp/mage-test" }).unwrap();
        ...
        let file2 = OpenOptions::new().read(true).write(true).open(if cfg!(target_os = "windows") { "%TEMP%\\mage-test" } else { "/tmp/mage-test" }).unwrap();
